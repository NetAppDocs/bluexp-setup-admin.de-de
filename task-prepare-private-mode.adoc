...
NOTE: Wenn Sie BlueXP in der verwenden möchten https://["AWS Commercial Cloud Services (C2S) Umgebung"^] Folgen Sie dann den separaten Anweisungen, die beschreiben, wie Sie den Konnektor vorbereiten, installieren und Cloud Volumes ONTAP starten. https://["Erste Schritte mit Cloud Volumes ONTAP in der AWS C2S Umgebung"^]
...
| Managen von Ressourcen in AWS. Der genaue Endpunkt hängt von der Region ab, in der Sie den Connector bereitstellen. https://["Details finden Sie in der AWS-Dokumentation"^] 
...
https://["Azure-Dokumentation: Öffentliche IP-SKU"^]
...
+
Weitere Informationen finden Sie unter https://["Microsoft Azure-Dokumentation: Erforderliche Berechtigungen"^].
...
.Schritt
. https://["Aktivieren Sie die folgenden Google Cloud APIs in Ihrem Projekt"^]
+
