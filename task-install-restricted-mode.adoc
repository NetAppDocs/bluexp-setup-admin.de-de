...
[role="lead"]
Stellen Sie den Connector im eingeschränkten Modus bereit, sodass Sie BlueXP  mit eingeschränkter Outbound-Verbindung zur SaaS-Ebene (BlueXP  Software as a Service) verwenden können. Installieren Sie den Connector, richten Sie BlueXP über die Benutzeroberfläche ein, die auf dem Connector ausgeführt wird, und stellen Sie dann die zuvor festgelegten Cloud-Berechtigungen bereit.
...
.Bevor Sie beginnen
Die Person, die den BlueXP -Konnektor eingerichtet hat, muss sich mit einem Login bei BlueXP  anmelden, das nicht zu einer BlueXP -Organisation gehört, die im Standardmodus arbeitet.

Wenn Ihre BlueXP -Anmeldung mit einer Organisation verknüpft ist, die im Standardmodus arbeitet, müssen Sie sich mit einem neuen BlueXP -Login oder mit den Anmeldedaten der NetApp Support-Website anmelden, die keiner Organisation angehören. Andernfalls wird die Option zum Aktivieren des eingeschränkten Modus auf dem Setup-Bildschirm nicht angezeigt.
