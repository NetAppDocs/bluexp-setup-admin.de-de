...
* Das Konto kann nicht mit der Cloud Volumes ONTAP-Lizenzierung verwendet werden.
* Digital Advisor und Support-Case-Management können nur ein NSS-Konto pro Benutzer zugeordnet werden.


Wenn Sie mit Cloud Volumes ONTAP arbeiten, können Sie die Zugangsdaten für die NetApp Support Website mit dem BlueXP Konto verknüpfen, dem Sie angehören. Mit den Zugangsdaten für NSS Konten können Sie Cloud Volumes ONTAP implementieren, wenn Sie Ihre eigene Lizenz (BYOL) verwenden, PAYGO-Systeme registrieren und die Cloud Volumes ONTAP Software aktualisieren.
