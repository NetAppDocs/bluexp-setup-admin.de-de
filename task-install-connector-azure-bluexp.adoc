...
[role="lead"]
Sie können einen Connector in Azure direkt aus BlueXP  erstellen. Um einen Connector in Azure von BlueXP  zu erstellen, müssen Sie Ihr Netzwerk einrichten, eine Azure-Rolle für die Bereitstellung des Connectors vorbereiten und dann den Connector implementieren.
