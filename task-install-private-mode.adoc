...
Wenn Sie BlueXP in der verwenden möchten https://["AWS Commercial Cloud Services (C2S) Umgebung"^] Folgen Sie dann den einzelnen Anweisungen, um in dieser Umgebung zu beginnen. https://["Erste Schritte mit Cloud Volumes ONTAP in der AWS C2S Umgebung"^]
...
----
. Laden Sie die Connector-Software von der herunter https://["NetApp Support Website"^]
+
...
.Schritte
. Öffnen Sie einen Webbrowser, und geben Sie ein https://[] Wobei _ipaddress_ die IP-Adresse des Linux-Hosts ist, auf dem Sie den Connector installiert haben.
+
