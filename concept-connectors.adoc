...
Wenn Sie BlueXP  im Standardmodus ausführen, müssen Sie keinen Konnektor installieren, um einige BlueXP -Dienste wie Software-Updates und Digital Wallet auszuführen. Für einige der Premium-Services von BlueXP  wie Ransomware-Schutz oder Disaster Recovery muss jedoch ein Connector installiert werden. Wenn Sie BlueXP  im eingeschränkten oder privaten Modus verwenden, müssen Sie einen Connector installieren.



== Was Sie im Standardmodus auf einen Connector verzichten können

Mit BlueXP  können Sie Ihre Storage-Ressourcen zentral managen, ohne einen Connector zu installieren, wenn Sie im Standard-Modus ausgeführt werden.
...
+
Zum Hinzufügen einer Lizenz zur digitalen Brieftasche ist nur ein Connector erforderlich, wenn Cloud Volumes ONTAP _Node-based_ Lizenzen verwendet werden. Da BlueXP  die auf Ihren Cloud Volumes ONTAP Systemen installierten Lizenzdaten erreichen muss, ist ein Connector erforderlich.
...
Wenn Sie BlueXP  im Standardmodus verwenden, müssen Sie einen oder mehrere Connectors installieren, um die folgenden Funktionen und Dienste in BlueXP  zu aktivieren:
...
* Disaster Recovery
* E-Series Systeme
* Wirtschaftliche Effizienz (schreibgeschützt)
* Google Cloud Storage Buckets
* On-Premises-ONTAP-Cluster-Integration in BlueXP-Datenservices
* Ausfallsicherheit (schreibgeschützt)
...
Sie können Connectors je nach Bedarf in einer unterstützten Hyperscaler-Umgebung oder vor Ort installieren.

* Amazon Web Services
* Microsoft Azure
+
Implementieren Sie den Connector in derselben Azure-Region wie die von ihm gemanagten Cloud Volumes ONTAP-Systeme oder in der https://docs.microsoft.com/en-us/azure/availability-zones/cross-region-replication-azure#azure-cross-region-replication-pairings-for-all-geographies["Azure Region Paar"^] für die Cloud Volumes ONTAP-Systeme. So wird sichergestellt, dass eine Azure Private Link-Verbindung zwischen Cloud Volumes ONTAP und ihren zugehörigen Storage-Konten verwendet wird. https://docs.netapp.com/us-en/bluexp-cloud-volumes-ontap/task-enabling-private-link.html["Erfahren Sie, wie Cloud Volumes ONTAP einen privaten Azure Link nutzt"^]
...
Sie können denselben Connector verwenden, um mehrere Arbeitsumgebungen in BlueXP  zu verwalten. Die maximale Anzahl von Arbeitsumgebungen, die ein einzelner Connector managen sollte, variiert. Das hängt von der Art der Arbeitsumgebungen, der Anzahl der Volumes, der zu verwaltenden Kapazität und der Anzahl der Benutzer ab.

Nutzen Sie eine umfangreiche Implementierung, arbeiten Sie mit Ihrem NetApp Ansprechpartner zusammen, um die Größe Ihrer Umgebung zu dimensionieren.

Eine weitere Überlegung bei der Ermittlung der erforderlichen Anzahl von Connectors ist der Speicherort. Wenn Sie beispielsweise Cloud Volumes ONTAP sowohl in Google Cloud als auch in Azure haben, benötigen Sie für jede Umgebung einen Connector. Wenn Ihr Storage vollständig vor Ort gespeichert ist, können Sie den Connector in einem beliebigen Hyperscaler oder On-Premises ausführen.
