...
+
Obwohl ein Connector nicht zum Erstellen einer Arbeitsumgebung erforderlich ist, ist er für die Erstellung und das Management von Volumes, die Replizierung von Daten und die Integration von FSX for ONTAP mit BlueXP Services wie Data Sense und Cloud Sync erforderlich.
...
* Sie verfügen über eine Multi-Cloud-Umgebung (AWS und Azure) und bevorzugen einen Connector in AWS und einen weiteren in Azure. Jedes managt die Cloud Volumes ONTAP Systeme, die in diesen Umgebungen ausgeführt werden.
