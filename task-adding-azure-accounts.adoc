...
.. Klicken Sie auf *Benachrichtigungen* und klicken Sie auf den Link in der Benachrichtigung, um Ihr Konto zu konfigurieren.
.. Klicken Sie Auf *Konto Konfigurieren*.
+
...
+
image:screenshot-azure-subscription.png["Ein Screenshot der Seite „Subscription Assignment“, auf der Sie dieses Abonnement mit einem oder mehreren Konten verknüpfen können, und wählen Sie, das vorhandene Abonnement für eines dieser Konten zu ersetzen."]
