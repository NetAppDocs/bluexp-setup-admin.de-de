...
image:screenshot-edit-name.png["Ein Screenshot, der die Möglichkeit zeigt, Ihren Namen unter Benutzereinstellungen zu bearbeiten."]
...
=== Anforderung für den Metadatenservice für die Amazon EC2 Instanz

Sie sollten beachten, dass IMDSv1 auf EC2-Instanzen erforderlich ist, bevor Sie die Connector-Software manuell in AWS installieren.

link:reference-limitations.html["Erfahren Sie mehr über die Anforderung an IMDSv1 in AWS"].
